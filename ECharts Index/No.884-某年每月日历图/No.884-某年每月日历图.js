...
const colors = ['#c4332b', '#16B644', '#6862FD', '#FDC763', '#FDC763'];
const iconIndex = [[1, 2], [0], [3], [1, 4], [1], [2], [3, 1], [1], [], [4], [3, 0], [0, 1]];
const itemIndex = [
    ['提高销售量类', '巩固消费群体类'],
    ['提高周转率类'],
    ['新品推广类'],
...
const seriesName = ['提高周转率类', '提高销售量类', '巩固消费群体类', '新品推广类', '扩展会员类'];

init();

function getMonth(date) {
    const temp = echarts.format.formatTime('yyyy-MM-dd', date);
    let text;
    if (temp.indexOf('08') > 0) {
        text = '05';
    } else if (temp.indexOf('09') > 0) {
        text = '06';
    } else if (temp.indexOf('10') > 0) {
        text = '07';
    } else if (temp.indexOf('11') > 0) {
        text = '08';
    } else if (temp.indexOf('15') > 0) {
        text = '09';
    } else if (temp.indexOf('16') > 0) {
        text = '10';
    } else if (temp === '2017-03-17') {
        text = '11';
    } else if (temp.indexOf('18') > 0) {
        text = '12';
    } else {
...
function getVirtulData() {
    ...
    const end = +echarts.number.parseDate(`${year}-03-19`);
    const dayTime = 3600 * 24 * 1000;
    const data = [];
    let shapeIndex = -1;
    for (let time = date; time < end; time += dayTime) {
        const temp = echarts.format.formatTime('yyyy-MM-dd', time);
        if (
            temp !== '2017-03-05' &&
            temp !== '2017-03-06' &&
            temp !== '2017-03-07' &&
            ...
            data.push([
                ...
                iconIndex[shapeIndex].join('|'),
                itemIndex[shapeIndex],
            ]);
        }
    }
    return data;
...
function renderItem(params, api) {
    ...
    if (Number.isNaN(cellPoint[0]) || Number.isNaN(cellPoint[1])) {
        return;
    }
        ...
        children:
            (layouts[events.length - 1] || []).map(function (itemLayout, index) {
                return {
                    ...
                    position: [
                        cellPoint[0] +
                            echarts.number.linearMap(itemLayout[0], [-0.5, 0.5], [-cellWidth / 2, cellWidth / 2]),
                        cellPoint[1] +
...
function init() {
    ...
    myChart.on('mouseover', function (params) {
        myChart.setOption({
            tooltip: {
                formatter() {
                    return `${getMonth(params.value[0])}月<br/>${params.event.target.name}`;
                },
            },
        });
    });
