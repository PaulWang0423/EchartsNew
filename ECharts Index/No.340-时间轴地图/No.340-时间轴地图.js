...
function initGeoJSON(geoJSON) {
    const arrGeoJSON = geoJSON.features.map((item) => {
        return {
    ...
    // 从大到小排序
    return arrGeoJSON.sort((a, b) => {
        return b.value - a.value;
    });
}

// 根据时间轴重构geoJSON数据
function initTimeTile(timeTitle, geoJSON) {
    let obj = {};
    timeTitle.forEach((item) => {
        obj[item] = initGeoJSON(geoJSON);
    });
    return obj;
}
const constructGeoJSON = initTimeTile(timeTitle, geoJSON);

console.log(constructGeoJSON);
...
timeTitle.forEach((item) => {
    option.options.push({
        visualMap: {
            show: false,
            left: 'left',
            max: constructGeoJSON[item][0].value,
            min: 0,
            inRange: {
                color: ['#FFF6D7', '#FFEBAF', '#FFDC88', '#FFCF69', '#FEB939'],
            },
            calculable: true,
        },
        series: [
