const CubeLeft = echarts.graphic.extendShape({
    ...
    buildPath: function (ctx, shape) {
        ...
        const c0 = [shape.x+18, shape.y - 9];  //右上
        const c1 = [shape.x+9, shape.y];  //左上
        const c2 = [xAxisPoint[0]+9, xAxisPoint[1]];  //左下
        const c3 = [xAxisPoint[0]+18, xAxisPoint[1] - 9];   //右下
...
const CubeTop = echarts.graphic.extendShape({
    ...
    buildPath: function (ctx, shape) {
        const c1 = [shape.x+18, shape.y - 9];   //右上
        const c2 = [shape.x, shape.y - 9]; //左上
        const c3 = [shape.x-9, shape.y]; //左下
        const c4 = [shape.x+9, shape.y];  //右下
        ctx.moveTo(c1[0], c1[1]).lineTo(c2[0], c2[1]).lineTo(c3[0], c3[1]).lineTo(c4[0], c4[1]).closePath();
    },
});
